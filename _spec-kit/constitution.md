...
version: 1.0.0
layer: SPEC-KIT
purpose: Authoring rules and quality bar for Awesome Second Brain enhancements. Loaded by the builder domain agent on every activation.
...
Central reference for building enhancements to the Awesome Second Brain (ASB) vault. The `builder` domain agent loads this file on every activation and enforces these rules across `/asb-specify`, `/asb-plan`, `/asb-build`, `/asb-test`, `/asb-ship`. See also [[CLAUDE]] and [[README|Spec-Kit README]].
...
Run the full `/asb-*` cycle for **non-trivial** changes — defined as any change that meets **at least one** of:
...
|---|---|---|---|
| 1. Specify | `/asb-specify <desc>` | `spec.md` | User reviews spec before planning |
| 2. Plan | `/asb-plan` | `plan.md`, `impact.md` | User approves impact list before building |
| 3. Build | `/asb-build` | `tasks.md`, repo edits | All tasks checked off + impact.md files touched |
| 4. Test | `/asb-test` | `test-plan.md`, `test-artifacts.md`, `test-results.md` | Every stage scored ≥3/5 |
| 5. Ship | `/asb-ship` | `ship.md`, commit | User confirms cleanup + commit |

No phase may be skipped. `/asb-test` is **mandatory** — a feature is not done until the test plan is scored.
...
If during `/asb-build` you discover a new impacted file, **pause**, update `impact.md`, and confirm with the user before continuing.
...
`/asb-ship` reads `git status` and groups changes by bucket. The user commits only the repo-bound bucket.
...
- Every behavior change updates `.claude/CLAUDE.md` **and** the relevant `.claude/core/system/*.md` file.
- `_spec-kit/features/NNN-*/` is never edited by hand during phases — only by the `/asb-*` commands.

---

## 8. Builder agent enforcement contract

When any `/asb-*` command is invoked, the agent must:
...
3. Refuse to run `/asb-ship` while `test-results.md` has any stage scored <3 or missing a score.
