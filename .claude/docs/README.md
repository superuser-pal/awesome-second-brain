...
```
→ Updated work/org/people/Sarah Chen.md with meeting context
→ Created work/1-1/Sarah 2026-03-26.md with key takeaways
→ Created Decision Record: "Defer Redis migration to Q2"
→ Added to work/review/Wins Doc.md: "Auth architecture praised by manager"
...
# → contact-importer creates notes for new people involved
# → Full timeline, root cause analysis, wins doc entry
```
...
# → updates indexes
# → wins-capture finds uncaptured wins
...
**Vault-first memory** keeps context across sessions and machines. All durable knowledge lives in `user/` topic notes (git-tracked, Obsidian-browsable, linked). Claude Code's `MEMORY.md` (`~/.claude/`) is an auto-loaded index that points to vault locations — never the storage itself. This means memories survive machine changes and are part of the graph.
...
| `contact-importer` | Bulk creates/updates person notes from Slack profiles | `/incident-capture` |
...
1. **Competency notes** in `work/review/competencies/` define your org's competency framework — one note per competency
...
4. **Wins Doc** aggregates wins per quarter with links to evidence notes
5. **`/peer-scan`** deep-scans a colleague's GitHub PRs and writes structured evidence to `work/review/evidence/`
6. **`/review-brief`** generates a full review brief by aggregating everything: wins entries, decisions, incidents, competency evidence, and 1:1 feedback
...
| People Directory | Everyone in `work/org/people/` with role, team |
...
`dashboards/HOME.md` embeds these views, making it the vault's dashboard.
...
5. Fill in **`user/NORTH_STAR.md`** with your goals — this grounds every session
...
dashboards/
  ...
  TASKS.md              Master task list — aggregated from all domain projects
...
work/review/
  Wins Doc.md           Running log of wins, linked to evidence
  wins/                 Quarterly wins notes (one per quarter)
...
user/
  NORTH_STAR.md         Goals and focus areas — read every session
  MEMORIES.md           Index of memory topics
  KEY_DECISIONS.md      Significant decisions and their reasoning
...
| What | Where |
|------|-------|
| Your goals | `user/NORTH_STAR.md` — grounds every session |
| Your org | `work/org/` — add your manager, team, key collaborators |
| Your competencies | `work/review/competencies/` — match your org's framework |
